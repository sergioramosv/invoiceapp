  ...
  return (
    <div className="bg-surface rounded-2xl shadow-sm border border-border p-8 space-y-6">
      <div className="text-center">
        ...
        <p className="text-text-secondary mt-1">{t('auth.signupSubtitle')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="name" className="block text-sm font-medium mb-1.5">{t('auth.name')}</label>
          <input
            id="name"
            type="text"
            required
        ...
        <div>
          <label htmlFor="email" className="block text-sm font-medium mb-1.5">{t('auth.email')}</label>
          <input
            id="email"
            type="email"
            required
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3 bg-text text-surface rounded-lg font-medium hover:bg-text-secondary transition-colors disabled:opacity-50"
        >
      ...
      <p className="text-center text-sm text-text-secondary">
        {t('auth.hasAccount')}{' '}
        <Link href="/login" className="text-text font-medium underline underline-offset-4 hover:text-primary transition-colors">{t('auth.login')}</Link>
      </p>
    </div>
  )
}
