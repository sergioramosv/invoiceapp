...
function DemoBanner() {
  const { t } = useI18n()
  return (
    <div className="bg-accent text-white text-center py-2 px-4 text-sm font-medium flex items-center justify-center gap-3">
      <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        <path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
      </svg>
      <span>{t('demo.bannerOpen')}</span>
    </div>
  )
}
...
export default function DemoLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <DemoProvider>
      <DemoBanner />
      <div className="min-h-[calc(100vh-36px)] bg-surface-secondary flex flex-col lg:flex-row">
        {/* Desktop sidebar — identical to workspace */}
        <aside className="hidden lg:flex lg:flex-col lg:w-60 lg:shrink-0 lg:h-[calc(100vh-36px)] lg:sticky lg:top-[36px] border-r border-border bg-surface overflow-y-auto">
          {/* Logo */}
          ...
          <div className="border-t border-border px-4 py-4 space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-text-secondary truncate">demo@invoiceapp.es</p>
            </div>
            <Link
              href="/"
              className="block w-full px-3 py-2 text-sm text-text-secondary border border-border rounded-lg hover:bg-surface-tertiary transition-colors text-center"
            >
              {t('nav.logout')}
            </Link>
          </div>
