...
import { createInitialState, getSubtotal, getDiscountAmount, getTaxAmount, getTotal, getBalanceDue, getVatBreakdown } from '@/types/invoice'
import { useI18n } from '@/lib/i18n'

export default function DemoNewInvoicePage() {
  const { addInvoice, getNextInvoiceNumber } = useDemo()
  const { t } = useI18n()
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="border-b border-border bg-surface px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">{t('editor.newDocument') || 'Nuevo documento'}</h1>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex lg:hidden border border-border rounded-lg overflow-hidden">
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium transition-colors ${
                activeTab === 'edit' ? 'bg-text text-surface' : 'bg-surface text-text-secondary hover:bg-surface-tertiary'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium transition-colors ${
                activeTab === 'preview' ? 'bg-text text-surface' : 'bg-surface text-text-secondary hover:bg-surface-tertiary'
              }`}
            >
          ...
          >
            ...
            <span className="hidden sm:inline">
              {downloading ? (t('editor.generating') || 'Generando...') : (t('editor.downloadPDF') || 'PDF')}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className={`w-full lg:w-[65%] xl:w-[70%] overflow-y-auto p-4 sm:p-6 bg-surface ${
        ...
        <div className={`w-full lg:w-[35%] xl:w-[30%] overflow-y-auto p-4 bg-surface-tertiary border-l border-border ${
