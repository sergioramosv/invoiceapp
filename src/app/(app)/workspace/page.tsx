...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (gridInitialized.current || !gridRef.current || loadingData) return
  ...
  const presupuestosAceptados = useMemo(
    () => allQuotes.filter(q => q.status === 'paid').length,
    [allQuotes]
  )

  const tasaAceptacion = presupuestosEmitidos > 0
    ? Math.round((presupuestosAceptados / presupuestosEmitidos) * 100)
    : 0
  ...
  const statusData = useMemo(() => {
    ...
    return Object.entries(counts)
      ...
      .map(([status, count]) => ({
        ...
        value: count,
        fill: STATUS_COLORS[status as InvoiceStatus] || '#a3a3a3',
      }))
  }, [invoices, t])

  /* Monthly trend (invoices + quotes count, last 6 months) */
  const monthlyTrend = useMemo(() => {
    const invMap: Record<string, number> = {}
    const quoMap: Record<string, number> = {}
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      result.push({
        ...
        facturas: invMap[key] || 0,
        presupuestos: quoMap[key] || 0,
      })
    }
    return result
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div ref={gridRef} className="grid-stack">
        ...
        <div className="grid-stack-item" gs-x="0" gs-y="0" gs-w="3" gs-h="2" gs-min-w="2" gs-min-h="2">
          <div className="grid-stack-item-content">
            <KPICard
              ...
              subtitle={t('dashboard.paidInvoices')}
              color="bg-success/15"
              icon={
        ...
        {/* KPI 2: Facturas emitidas */}
        <div className="grid-stack-item" gs-x="3" gs-y="0" gs-w="3" gs-h="2" gs-min-w="2" gs-min-h="2">
          <div className="grid-stack-item-content">
            <KPICard
              label={t('dashboard.invoicesIssued')}
              value={facturasEmitidas}
              subtitle={`${allInvoices.filter(i => i.status === 'paid').length} ${t('dashboard.paid')}`}
              color="bg-accent/15"
              icon={
        ...
        {/* KPI 3: Presupuestos emitidos */}
        <div className="grid-stack-item" gs-x="6" gs-y="0" gs-w="3" gs-h="2" gs-min-w="2" gs-min-h="2">
          <div className="grid-stack-item-content">
            <KPICard
              label={t('dashboard.quotesIssued')}
              value={presupuestosEmitidos}
              subtitle={`${allQuotes.filter(q => q.status === 'sent').length} ${t('dashboard.sent')}`}
              color="bg-warning/15"
              icon={
        ...
        {/* KPI 4: Presupuestos aceptados */}
        <div className="grid-stack-item" gs-x="9" gs-y="0" gs-w="3" gs-h="2" gs-min-w="2" gs-min-h="2">
          <div className="grid-stack-item-content">
            <KPICard
              label={t('dashboard.quotesAccepted')}
              value={presupuestosAceptados}
              subtitle={presupuestosEmitidos > 0 ? `${tasaAceptacion}% ${t('dashboard.conversionRate')}` : undefined}
              color="bg-success/15"
              icon={
                <svg className="w-5 h-5 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div className="flex-1 min-h-0">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyRevenue} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
                    ...
                    <YAxis tick={{ fontSize: 11, fill: 'var(--color-text-muted)' }} axisLine={false} tickLine={false} width={50}
            ...
            <div className="h-full bg-surface border border-border rounded-xl p-5 flex flex-col">
              ...
              <div className="flex-1 overflow-auto space-y-2">
                {(() => {
                  ...
                  allInvoices
                    .filter(i => i.status === 'paid' && i.billToName)
                    .forEach(i => {
                      clientTotals[i.billToName!] = (clientTotals[i.billToName!] || 0) + (i.total || 0)
                    })
                  ...
                  return sorted.map(([name, total]) => (
                    <div key={name} className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-text-muted shrink-0 ml-2">{formatCurrency(total)}</span>
                      </div>
          ...
          <div className="grid-stack-item-content">
            <div className="h-full bg-surface border border-border rounded-xl flex flex-col overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-border">
                ...
                <Link href="/workspace/invoices" className="text-xs text-accent hover:text-accent-light transition-colors font-medium">
                  {t('dashboard.viewAll')}
                </Link>
              </div>
                          ...
                          <tr key={doc.id} className="border-b border-border-light last:border-0 hover:bg-surface-secondary transition-colors">
                            <td className="py-2 px-4">
                              <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold ${
                                ...
                                {isQuote ? t('docType.quote') : t('docType.invoice')}
                              </span>
                            </td>
                            <td className="py-2 px-4 font-medium text-text text-xs">
                              <Link href={`/workspace/edit/${doc.id}`} className="hover:text-accent transition-colors">
                                {doc.invoiceNumber || '---'}
                              </Link>
                            </td>
                            <td className="py-2 px-4 text-text-secondary text-xs">{doc.billToName || '---'}</td>
                            <td className="py-2 px-4 text-right font-medium text-text text-xs">
                              {formatCurrency(doc.total || 0, doc.currency || 'EUR')}
                            </td>
                            <td className="py-2 px-4 text-center">
                              <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-medium ${statusCfg.className}`}>
                                {statusCfg.label}
                              </span>
                            </td>
                          </tr>
