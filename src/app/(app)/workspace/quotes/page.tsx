...
type FilterTab = 'all' | 'sent' | 'accepted'
...
export default function QuotesPage() {
  ...
  const STATUS_CONFIG: Record<InvoiceStatus, { label: string; className: string }> = {
    ...
    paid: { label: t('quotes.accepted'), className: 'bg-success/10 text-success' },
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const filename = pdfInvoice.invoiceNumber ? `presupuesto-${pdfInvoice.invoiceNumber}.pdf` : 'presupuesto.pdf'
  ...
  async function confirmConvert() {
    ...
    try { await convertQuoteToInvoice(convertTarget); await fetchData() } catch (err) { console.error(err) }
  ...
  const filtered = useMemo(() => {
    ...
    if (filterTab === 'sent') list = list.filter(q => q.status !== 'paid')
    if (filterTab === 'accepted') list = list.filter(q => q.status === 'paid')
  ...
  const quotes = allDocs
  const totalPresupuestado = quotes.reduce((s, q) => s + (q.total || 0), 0)
  const sentCount = quotes.filter(q => q.status !== 'paid').length
  const acceptedCount = quotes.filter(q => q.status === 'paid').length
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-secondary mt-1">{quotes.length} {t('filter.quotes').toLowerCase()}</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => exportToCSV(filtered, 'presupuestos')} className="px-4 py-2 text-sm border border-border rounded-lg font-medium hover:bg-surface-tertiary transition-colors flex items-center gap-2">
      ...
      {quotes.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-surface border border-border rounded-xl p-4">
            <p className="text-sm text-text-muted">{t('quotes.totalQuoted')}</p>
            <p className="text-2xl font-bold mt-1">{formatCurrency(totalPresupuestado, 'EUR')}</p>
          </div>
          <div className="bg-surface border border-border rounded-xl p-4">
            <p className="text-sm text-text-muted">{t('quotes.issued')}</p>
            <p className="text-2xl font-bold mt-1">{sentCount}</p>
          </div>
          <div className="bg-surface border border-border rounded-xl p-4">
            <p className="text-sm text-text-muted">{t('quotes.accepted')}</p>
      ...
      {quotes.length > 0 && (
        <div className={styles.filterTabs}>
          <button onClick={() => setFilterTab('all')} className={`${styles.filterTab} ${filterTab === 'all' ? styles.filterTabActive : ''}`}>
            {t('filter.all')} ({quotes.length})
          </button>
          <button onClick={() => setFilterTab('sent')} className={`${styles.filterTab} ${filterTab === 'sent' ? styles.filterTabActive : ''}`}>
            {t('quotes.issued')} ({sentCount})
          </button>
          <button onClick={() => setFilterTab('accepted')} className={`${styles.filterTab} ${filterTab === 'accepted' ? styles.filterTabActive : ''}`}>
            {t('quotes.accepted')} ({acceptedCount})
          </button>
        </div>
      )}

      {loadingData ? <SkeletonTable rows={5} /> : filtered.length === 0 && quotes.length === 0 ? (
        ...
        <div className="bg-surface border border-border rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={6} className="py-8 text-center text-text-muted">{t('workspace.noResults')}</td></tr>
                ) : filtered.map(quote => {
                  ...
                  return (
                    <tr key={quote.id} className="border-b border-border last:border-0 hover:bg-surface-secondary transition-colors">
                      <td className="py-3 px-4 font-medium"
                        onMouseEnter={e => setHoverPreview({ invoice: quote, x: e.clientX, y: e.clientY })}
                        onMouseMove={e => setHoverPreview(prev => prev?.invoice.id === quote.id ? { ...prev, x: e.clientX, y: e.clientY } : prev)}
                      ...
                      <td className="py-3 px-4 text-center">
                        <select value={quote.status} onChange={async e => {
                          const s = e.target.value as InvoiceStatus
                          await updateInvoice(quote.id, { status: s })
                          setAllDocs(prev => prev.map(q => q.id === quote.id ? { ...q, status: s } : q))
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => setPdfInvoice(quote)} className="p-1.5 text-text-secondary hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" title={t('action.downloadPDF')}>
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                          </button>
                          <button onClick={() => setConvertTarget(quote)} className="p-1.5 text-text-secondary hover:text-success hover:bg-success/10 rounded-lg transition-colors" title={t('action.convertToInvoice')}>
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                          </button>
                          <Link href={`/workspace/edit/${quote.id}`} className="p-1.5 text-text-secondary hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" title={t('action.edit')}>
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                          </Link>
                          <button onClick={() => handleDuplicate(quote)} className="p-1.5 text-text-secondary hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" title={t('action.duplicate')}>
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                          </button>
                          <button onClick={() => setDeleteTarget(quote.id)} className="p-1.5 text-text-secondary hover:text-danger hover:bg-danger/10 rounded-lg transition-colors" title={t('action.delete')}>
      ...
      <ConfirmModal open={!!convertTarget} onClose={() => setConvertTarget(null)} onConfirm={confirmConvert} title={t('modal.convertInvoice')} message={t('modal.convertInvoiceMsg')} confirmLabel={t('modal.convertBtn')} />
    </div>
  )
}
